...
import { HTTP_REST_PORT, HTTP_WS_PORT, OLD_BIP44_PATH, REST_API, TEST_NODE } from '../consts';
...
export class SDK {
    ...
    static createWallet(name: string,
        ...
        wallet.defaultoldId = identity.oldId;
        ...
        const tx = buildRegisteroldIdTx(identity.oldId, publicKey, gasPrice, gasLimit);
    ...
    static importIdentityWithPrivateKey(
        ...
        const tx = buildGetDDOTx(identity.oldId);
        const restClient = new RestClient(`http://${SDK.SERVER_NODE}:${SDK.REST_PORT}`);
        return restClient.sendRawTransaction(tx.serialize(), true).then((res: any) => {
            ...
            if (result.Result) {
                ...
                obj.error = ERROR_CODE.UNKNOWN_oldId;
                obj.result = '';
            }
    ...
    static signSelfClaim(
        ...
        const publicKeyId = oldId + '#keys-1';
    ...
    static importAccountMnemonic(label: string, mnemonic: string, password: string, callback?: string) {
        ...
        const pri = hdkey.derive(OLD_BIP44_PATH);
