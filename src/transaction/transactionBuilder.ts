...
import { makeTransferTx } from '../smartcontract/nativevm/OldAssetTxBuilder';
import { buildGetDDOTx, buildRegisteroldIdTx } from '../smartcontract/nativevm/oldIdContractTxBuilder';
...
export function buildNativeTxFromJson(json: any) {
    ...
    if (json.contractHash.indexOf('02') > -1 || json.contractHash.indexOf('01') > -1) { // OLD2 ONG contract
        const tokenType = json.contractHash.indexOf('02') > -1 ? 'OLD1' : 'OLD2';
    ...
    } else if (json.contractHash.indexOf('03') > -1) { // OLD1 ID contract
        if (funcArgs.operation === 'regIDWithPublicKey') {
            const oldId = args[0].value.substr(args[0].value.indexOf(':') + 1);
            const pk = new PublicKey(args[1].value.split(':')[1]);
            const payer = new Address(json.payer);
            const tx = buildRegisteroldIdTx(oldId, pk, json.gasPrice, json.gasLimit, payer);
            return tx;
