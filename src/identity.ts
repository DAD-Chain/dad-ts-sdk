...
export class Identity {
    ...
    static importIdentity(
        ...
        // generate oldId from p
        const publicKey = privateKey.getPublicKey();
        identity.oldId = Address.generateoldId(publicKey);
        identity.label = label;
        identity.lock = false;
        identity.isDefault = false;

        // control
        const control = new ControlData('1', encryptedPrivateKey, Address.fromoldId(identity.oldId), saltBase64);
    ...
    static create(privateKey: PrivateKey, keyphrase: string, label: string, params?: ScryptParams) {
        ...
        const publicKey = privateKey.getPublicKey();
        identity.oldId = Address.generateoldId(publicKey);
        const address = Address.fromoldId(identity.oldId);
