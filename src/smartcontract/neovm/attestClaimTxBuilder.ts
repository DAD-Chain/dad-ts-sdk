 ...
 * @param issuer Issuer's OLD ID
 * @param subject Subject's OLD ID
 ...
 *
 * @param claimId Unique id of the claim
 * @param revokeroldId Revoker's OLD ID
 * @param gasPrice Gas price
 * @param gasLimit Gas limit
 * @param payer Payer's address
 */
export function buildRevokeRecordTx(claimId: string, revokeroldId: string,
                                    gasPrice: string, gasLimit: string, payer: Address) {
    ...
    if (revokeroldId.substr(0, 3) === 'did') {
        revokeroldId = str2hexstr(revokeroldId);
    }

    const p1 = new Parameter(name1, type1, str2hexstr(claimId));
    const p2 = new Parameter(f.parameters[1].getName(), ParameterType.ByteArray, revokeroldId);
