...
export default class RpcClient {
    ...
    constructor( url ?: string ) {
        this.url = url || TEST_OLD_URL.RPC_URL;
    }
     ...
     * The result contains OLD1 and OLD2.
    ...
    /**
     * Get allowanece
     * @param asset Asset's type.Only OLD1 and OLD2 supported.
    ...
    getUnboundOng(address: Address): Promise<any> {
        const req = this.makeRequest('getunboundong', 'old2', address.toBase58(), address.toBase58());
        return axios.post(this.url, req).then((res) => {
            return res.data;
        });
    }
