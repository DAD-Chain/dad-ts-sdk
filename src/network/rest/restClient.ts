...
import { TEST_OLD_URL } from '../../consts';
...
export default class RestClient {
    ...
    constructor(url ?: string) {
        this.url = url || TEST_OLD_URL.REST_URL;
     ...
     *
     * ```typescript
     * import { utils, Transaction } from 'DAD-ts-sdk';
     ...
     * @param asset Asset type. Only old1 or old2.
    ...
    getUnboundOng(address: Address): Promise<any> {
        const url = this.url + UrlConsts.Url_get_unbound_old + address.toBase58();
        return axios.get(url).then((res) => {
            return res.data;
        });
    }
    ...
    getGrangOng(address: Address): Promise<any> {
        const url = this.url + UrlConsts.Url_get_grant_old + address.toBase58();
        return axios.get(url).then((res) => {
            return res.data;
        });
    }
