...
import { buildGetDDOTx, buildGetPublicKeyStateTx } from '../smartcontract/nativevm/oldIdContractTxBuilder';
...
export abstract class Message {
     ...
     *
     * @param url Restful endpoint of DAD node
    ...
    /**
     * Verifies the signature and check ownership of specified Old ID through smart contract call.
     *
     * @param url Restful endpoint of DAD node
    ...
    private verifyKeyOwnership(): boolean {
        ...
        if (signature !== undefined && signature.publicKeyId !== undefined) {
            const oldId = extractOldId(signature.publicKeyId);

            return oldId === this.metadata.issuer;
        } else {
            return false;
        }
    }
...
/**
 * Gets the state of public key associated with OLD ID from blockchain.
 *
 * @param publicKeyId The ID of a signature public key
 * @param url Restful endpoint of DAD node
 */
export async function retrievePublicKeyState(publicKeyId: string, url: string): Promise<PublicKeyStatus> {
    ...
    const client = new RestClient(url);
    const tx = buildGetPublicKeyStateTx(oldId, keyId);
    const response = await client.sendRawTransaction(tx.serialize(), true);
...
/**
 * Extracts Old ID from public key Id.
 *
 * @param publicKeyId The ID of a signature public key
 */
export function extractOldId(publicKeyId: string): string {
