...
import { OLD_TOTAL_SUPPLY, UNBOUND_GENERATION_AMOUNT, UNBOUND_TIME_INTERVAL, WEBVIEW_SCHEME } from './consts';
...
export function unboundDeadline() {
    ...
    if (UNBOUND_GENERATION_AMOUNT[numInterval - 1] !== 1 ||
        ! ((count - UNBOUND_TIME_INTERVAL < OLD_TOTAL_SUPPLY) && OLD_TOTAL_SUPPLY <= count)) {
        throw new Error('incompatible constants setting');
    }
    return UNBOUND_TIME_INTERVAL * numInterval - (count - OLD_TOTAL_SUPPLY);
}
